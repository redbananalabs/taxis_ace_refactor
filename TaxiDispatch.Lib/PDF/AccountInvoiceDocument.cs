            ...
            container.PaddingVertical(0).Column(column =>
            {
                column.Item().Row(row =>
                {
                    ...
                    row.RelativeItem().AlignRight().Column(col =>
                    {
                        col.Item().AlignRight().Width(200).Table(tbl =>
                        {
                            ...
                            tbl.Cell().Row(1).Column(1).AlignRight().PaddingRight(10).Text("INVOICE #:").SemiBold();
                            tbl.Cell().Row(1).Column(2).AlignLeft().Text(Model.InvoiceNumber);
                            tbl.Cell().Row(2).Column(1).AlignRight().PaddingRight(10).Text("DATE:").SemiBold();
                            tbl.Cell().Row(2).Column(2).Text(Model.InvoiceDate.ToString("dd/MM/yy"));
                            tbl.Cell().Row(3).Column(1).AlignRight().PaddingRight(10).Text("YOUR REF:").SemiBold();
                            tbl.Cell().Row(3).Column(2).Text(Model.Reference);
                            tbl.Cell().Row(4).Column(1).AlignRight().PaddingRight(10).Text("PO NUMBER:").SemiBold();
                            tbl.Cell().Row(4).Column(2).Text(Model.OrderNo);
                ...
                column.Item().Row(row =>
                {
                    row.RelativeItem().PaddingTop(-20).Column(col =>
                    {
                        ...
                        {
                            text.Line("INVOICE SUMMARY").Bold().FontSize(20);
                        });
                ...
                column.Item().Row(row =>
                {
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().AlignCenter().Text("Payments should be made within 30 days by bank transfer to the following account.").Bold();
                        col.Item().AlignCenter().Text(text =>
                        {
                        ...
                        col.Item().AlignCenter().PaddingTop(5).Text("Ace Taxis (Dorset) Ltd, 1 Briar Close, Gillingham, Dorset, SP8 4SS").Bold();
                    });
                });
